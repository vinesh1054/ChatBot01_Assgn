...
memory = ConversationBufferMemory(memory_key="history")  # Ensure memory key is "history"
...
chat_prompt = PromptTemplate(
    input_variables=["history", "input"],  # Ensure input variables match memory
    template="""
...
chat = ConversationChain(llm=chat_model, memory=memory, prompt=chat_prompt, verbose=True)
...
if st.button("Send"):
    if user_input:
        ...
        response = chat.run({"input": user_input})  # Ensure correct input format
